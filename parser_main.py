...
def parser():
    ...
    for countryIndex in range(1, 2): # не через in чтобы пропустить Алеутские острова # поменять на len(countries) при запуске на все страны
        ...
        # Если продолжаем парсинг, то убрать
        """
        ...
        for cnt in range(0,len(letters)):
            ...
            #======================= Работа над продолжением загрузки
            continue_downloading = False
            if os.path.exists(letter_path):
                dir_cities_and_rests = os.listdir(letter_path)
                dir_cities = os.listdir("1_"+ letter_path)

                for i in range(len(dir_cities)):
                    dir_cities[i] = dir_cities[i][:-4]

                for i in range(0, len(dir_cities)):
                    if dir_cities_and_rests[i] not in dir_cities:
                        #print(f"{dir_cities_and_rests[i]} ")
                        city_to_continue = dir_cities_and_rests[i]
                        continue_downloading = True


            #==============================
            if continue_downloading == False: # НОВАЯ СТРОКА
                if os.path.exists(letter_path):#and not(os.path.exists(next_letter_path)):
                    shutil.rmtree(letter_path)
                os.mkdir(letter_path)
            if continue_downloading == False: # НОВАЯ СТРОКА
                if os.path.exists("1_"+letter_path):# and not(os.path.exists(next_letter_path)):
            ...
            for cityHref in cityHrefs:
                ...
                for names in latin_names_list:
                    if names["name"] == city_guru.latin_name:
                        names["used"] += 1
                        city_guru.latin_name = city_guru.latin_name+"-"+str(names["used"])
                        flag = True
                if flag == False:
                    latin_names_dict = {
                        "name": city_guru.latin_name,
                        "used": 0
                    }
